...
LazyVim and patch the snacks.nvim picker config to show hidden files.
...
# ---------------------------------------------------------------------------
# snacks.nvim override
# ---------------------------------------------------------------------------

SNACKS_OVERRIDE_LUA = '''-- Generated by install_neovim.py
-- Override LazyVim's bundled snacks.nvim config to show hidden files
-- in the picker (files, grep) and the explorer.
--
-- Notes:
--   * `hidden = true` shows dotfiles (e.g. .env.example, .eslintrc).
--   * `ignored` is left at its default (false), so files matched by
--     .gitignore stay hidden. Flip to `true` if you want to see them.
--   * lazy.nvim deep-merges these `opts` into LazyVim's existing snacks
--     config; everything else (dashboard, notifier, indent, etc.) is
--     preserved untouched.
return {
  {
    "folke/snacks.nvim",
...
# Marker used to detect files this script has previously written, so we can
# distinguish our own generated config from a user-authored file with the
# same name and avoid clobbering the latter.
GENERATED_MARKER = "-- Generated by install_neovim.py"


def configure_snacks_hidden(config_dir: Path) -> Path:
    plugins_dir = config_dir / "lua" / "plugins"
    plugins_dir.mkdir(parents=True, exist_ok=True)

    out = plugins_dir / "snacks.lua"
    if out.is_file():
        try:
            current = out.read_text(encoding="utf-8")
        except OSError:
            current = None
        if current == SNACKS_OVERRIDE_LUA:
            LOG.info("Snacks override already present at %s; no changes needed.",
                     out)
            return out
        if current is not None and not current.startswith(GENERATED_MARKER):
            LOG.warning("Existing %s is user-authored (no generated marker); "
                        "refusing to overwrite. Skipping snacks config.", out)
            return out
        LOG.info("Existing %s differs from generated content; overwriting.", out)
    out.write_text(SNACKS_OVERRIDE_LUA, encoding="utf-8")
    LOG.info("Wrote snacks.nvim override to %s", out)
    return out
...
def print_followup(target: Target, bin_dir: Path, *, lazyvim_skipped: bool) -> None:
    ...
    if not lazyvim_skipped:
        ...
        print("snacks.picker is configured to show hidden files (find_files, grep, explorer).")
    print()


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Install Neovim + LazyVim with snacks.picker hidden-files enabled.",
    )
    parser.add_argument("-v", "--verbose", action="store_true",
                        help="Enable debug logging.")
    parser.add_argument("--skip-lazyvim", action="store_true",
                        help="Install only the Neovim binary; skip LazyVim and snacks steps.")
    ...
    try:
        ...
        configure_snacks_hidden(config_dir)
    except InstallError as e:
        LOG.error(str(e))
